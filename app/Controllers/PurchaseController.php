...
class PurchaseController extends Controller {
    ...
    // ============================================================
    // LEGACY / BASIC PURCHASE ORDER METHODS
    // ============================================================
    public function index(): void {
        $this->requireAuth();
        $p = $this->paginate(); $q = $this->get('q','');
        $where=["po.business_id=?"]; $params=[$this->bizId];
        if ($q) { $where[]='(po.reference LIKE ? OR c.name LIKE ?)'; $params=array_merge($params,["%$q%","%$q%"]); }
        $sql="SELECT po.*,c.name as supplier_name FROM purchase_orders po LEFT JOIN contacts c ON c.id=po.supplier_id WHERE ".implode(' AND ',$where)." ORDER BY po.created_at DESC";
        $result = DB::page($sql,$params,$p['page'],$p['per_page']);
        $this->view('purchases/index', compact('result','q'));
    }

    public function create(): void {
        $this->requireAuth();
        $suppliers = DB::all("SELECT id,name,code FROM contacts WHERE business_id=? AND type IN ('supplier','both') AND is_active=1 ORDER BY name", [$this->bizId]);
        ...
        if ($this->isPost()) {
            ...
            if (!$items) { flash('error','Add items.'); }
            else {
                $total = array_sum(array_column($items,'total'));
                $ref = 'PO-'.date('Ymd').'-'.str_pad((int)DB::val("SELECT COUNT(*)+1 FROM purchase_orders WHERE business_id=?",[$this->bizId]),4,'0',STR_PAD_LEFT);
                $id = DB::insert('purchase_orders',['business_id'=>$this->bizId,'reference'=>$ref,'supplier_id'=>$this->post('supplier_id'),'order_date'=>$this->post('order_date',date('Y-m-d')),'due_date'=>$this->post('due_date'),'total'=>$total,'paid_amount'=>0,'due_amount'=>$total,'payment_status'=>'unpaid','status'=>'draft','notes'=>$this->post('notes'),'created_by'=>$this->userId,'created_at'=>date('Y-m-d H:i:s')]);
                foreach ($items as $item) DB::insert('purchase_items',['purchase_id'=>$id,'product_id'=>$item['product_id'],'quantity'=>$item['qty'],'unit_cost'=>$item['price'],'total'=>$item['total']]);
                flash('success',"PO $ref created."); $this->redirect("/purchases/view/$id");
            }
        }
        $this->view('purchases/form', compact('suppliers','products'));
    }

    public function show(string $id): void {
        $this->requireAuth();
        $po = DB::row("SELECT po.*,c.name as supplier_name FROM purchase_orders po LEFT JOIN contacts c ON c.id=po.supplier_id WHERE po.id=? AND po.business_id=?",[$id,$this->bizId]);
        if (!$po) { flash('error','Not found.'); $this->redirect('/purchases'); }
        ...
        $this->view('purchases/view', compact('po','items'));
    }

    public function delete(string $id): void {
        $this->requireAuth();
        DB::q("UPDATE purchase_orders SET status='cancelled' WHERE id=? AND business_id=?",[$id,$this->bizId]);
        $this->json(true,'Cancelled.');
    }
    ...
    public function receive(string $id): void {
        ...
        $items = DB::all("SELECT * FROM purchase_items WHERE purchase_id=?",[$id]);
        foreach ($items as $item) {
            DB::q("INSERT INTO stock (business_id,product_id,location_id,quantity,avg_cost,updated_at) VALUES (?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE quantity=quantity+VALUES(quantity),updated_at=NOW()", [$this->bizId,$item->product_id,$this->locId,$item->quantity,$item->unit_cost]);
            DB::insert('stock_movements',['business_id'=>$this->bizId,'product_id'=>$item->product_id,'location_id'=>$this->locId,'type'=>'purchase','reference_type'=>'po','reference_id'=>$id,'quantity'=>$item->quantity,'unit_cost'=>$item->unit_cost,'created_by'=>$this->userId,'created_at'=>date('Y-m-d H:i:s')]);
        }
        DB::update('purchase_orders',['status'=>'received','received_date'=>date('Y-m-d')],'id=?',[$id]);
        $this->json(true,'Stock received!');
    }
